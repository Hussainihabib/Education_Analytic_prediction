      ...
      <div className="hidden lg:flex flex-col justify-between bg-navy-900 text-cream-100 px-14 py-12 relative overflow-hidden">
        ...
        <div className="relative">
          <div className="flex items-center gap-3">
            <div className="w-11 h-11 rounded-full border border-amber-accent flex items-center justify-center text-amber-accent font-serif text-lg">EP</div>
            <div>
              <div className="font-serif text-xl font-semibold">EduPredict</div>
              <div className="text-[10px] tracking-[0.2em] text-slate-400 font-mono">BIG DATA &amp; PREDICTIVE ANALYTICS</div>
            </div>
          </div>
          ...
          <p className="text-slate-300 mt-20 max-w-md leading-relaxed">
            EduPredict turns academic records, attendance, and LMS activity into early warnings — so no student falls through the cracks unnoticed.
          </p>
        ...
        <div className="relative border-t border-white/10 pt-5 space-y-3 font-mono text-sm">
          {ticker.map((t) => (
            <div key={t.label} className="flex items-center justify-between">
              ...
              <span className="text-teal-accent font-semibold">{t.value}</span>
            </div>
          ))}
        </div>
      </div>
